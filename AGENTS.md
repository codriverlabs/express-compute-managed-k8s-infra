...
│   └── 11-install-karpenter.sh  # Karpenter v1.8.2
...
| Install Karpenter | `./eks-d-setup/11-install-karpenter.sh` (part of install-all.sh) |
...
3. **Terraform for infrastructure**: Infrastructure uses Terraform (see `terraform/`)
4. **Karpenter v1 API**: Uses `karpenter.sh/v1` and `karpenter.k8s.aws/v1` (not v1beta1)

## Configuration Files

- **Terraform configs**: HCL in `terraform/`
